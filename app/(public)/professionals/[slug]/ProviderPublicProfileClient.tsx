...
import { withScheme, toWhatsAppLink } from '@/lib/social-links'
...
export function ProviderPublicProfileClient({ slug }: { slug: string }) {
    const router = useRouter()
    const pathname = usePathname()
    ...
    const { data: savedRecord } = useQuery({
        queryKey: ['saved-check', authProfile?.id, profile?.id],
        queryFn: async () => {
            if (!authProfile || !profile?.id) return null
        ...
        enabled: !!authProfile && !!profile?.id,
    })
    ...
    const toggleSave = useMutation({
        mutationFn: async () => {
            ...
            if (!authProfile || !profile?.id) return
            ...
            if (isSaved && savedRecord) {
                ...
                const { error } = await supabase
                    .from('saved_providers')
                    .insert({ client_id: authProfile.id, provider_id: profile.id })
                if (error) throw error
            }
        },
        onSuccess: () => {
    ...
    function handleBookClick(service: Service) {
        if (!session) {
            router.push(`/login?from=${encodeURIComponent(pathname)}&bookService=${service.id}`)
            return
        }
    ...
    const resumeBookServiceId = searchParams.get('bookService')
